...
<div class="col-sm-9 col-md-10 mt-5">
 <?php
  $sql = "SELECT * FROM assignwork_tb WHERE status = 'pending'";
     ...
     while ($row = $result->fetch_assoc()){
      echo '<tr>';
       ...
       echo '<td>';
        ...
        echo '<input type="hidden" name="id" value='.$row['request_id'].'><button class="btn btn-success" name="complete" value="Complete" type="submit"><i class="fas fa-check"></i></button>';
       echo '</form>';
       echo '</td>';
      echo '</tr>';
     }
  ...
  $sql = "UPDATE assignwork_tb SET status = 'completed', completion_date = NOW() WHERE request_id = {$_REQUEST['id']}";
  if($conn->query($sql) == TRUE){
   echo '<meta http-equiv="refresh" content= "0;URL=?completed" />';
  } else {
   echo "Unable to Mark Work as Complete";
  }
  }
 ?>
</div> <!-- End 2nd COlumn -->
